...
namespace Oxide.Plugins
{
	...
	{
		private const string databaseName = "IngameClockGUI";
		private const int isClockEnabled = 1 << 0;
		private const int isServerTime = 1 << 1;
		private const string defaultInfoSize = "0.3";
		...
		private void ShowInfo(string text, string iSize)
		{
			int size = BasePlayer.activePlayerList.Count;
			for(int i = 0; i < size; i++)
		...
		private void SaveData() => Interface.Oxide.DataFileSystem.WriteObject(databaseName, data);

		private bool GetOption(int options, int option)
		{
			if((options & option) != 0)
				return true;
			else
		...
		private void SendHelpText(BasePlayer player) => PrintToChat(player, (string)Config["Messages", "Help"]);
		...
		private string Replier(BasePlayer player, bool prefix, string message, params string[] args) => (prefix ? (lang.GetMessage(mprefix, this, player.UserIDString) + " ") : string.Empty) + string.Format(lang.GetMessage(message, this, player.UserIDString), args);
		...
		private const string menabled = "Enabled clock";
		private const string mdisabled = "Disabled clock";
		private const string mgametime = "Clock shows game time";
		private const string mservertime = "Clock shows server time";
		private const string mhelp = "Help message";
		private const string mrestrictedchanging = "Time choosing prevented";
		...
		{
			{ mprefix, "[Clock]" },
			{ menabled, "Clock indicator has been enabled." },
			{ mdisabled, "Clock indicator has been disabled." },
			{ mgametime, "Clock now shows game time for you." },
			{ mservertime, "Clock now shows server time for you." },
			{ mrestrictedchanging, "You do not have permission to choose between game and server time!" },
			{ mhelp, "Help message is here btw." }
		};
	}
}