...
>>>>>>> master
        ...
        HashSet<string> Deployables { get; } = new HashSet<string>();
        Timer ScheduleTimer { get; set; }
        ...
        protected override void LoadConfig()
        {
            ...
            try
            {
                ...
                else Puts("Configuration loaded...");
            }
            catch
            {
                LoadDefaultConfig();
        ...
        private string GetReply(BasePlayer player, string message, params object[] args)
        {
            return string.Format(lang.GetMessage(message, this, player?.UserIDString), args);
        }
        ...
        {
            if(Settings.ScheduledCleanup > 0) StartScheduleTimer();
        }
        ...
        private bool CommandHandler(Arg arg)
        {
            ...
            if(player && !permission.UserHasPermission(player.UserIDString, PERMISSION)) arg.ReplyWith(GetReply(player, mnoperm));
            else if(!arg.HasArgs()) ScheduledCleanup();
            else
            {
        ...
        {
            var deps = from def in ItemManager.GetItemDefinitions()
        ...
        {
            if(ScheduleTimer != null && !ScheduleTimer.Destroyed) ScheduleTimer.Destroy();
