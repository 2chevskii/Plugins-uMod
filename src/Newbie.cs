...
namespace Oxide.Plugins
{
    ...
    internal class Newbie : RustPlugin
    {
        ...
        private void Announcer(string message, params string[] args) =>
            Server.Broadcast(
                $"{lang.GetMessage(mprefix, this)} {string.Format(lang.GetMessage(message, this), args)}"
            );
        ...
        private void Replier(
            ...
            player.ChatMessage(
                $"{(prefix ? (lang.GetMessage(mprefix, this) + " ") : string.Empty)}{string.Format(lang.GetMessage(message, this), args)}"
            );
        ...
        private void Loaded()
        {
            try
            {
                allplayers = Interface.Oxide.DataFileSystem
                    .GetFile("Newbie")
        ...
        private void Unload() =>
            Interface.Oxide.DataFileSystem.GetFile("Newbie").WriteObject(allplayers);
