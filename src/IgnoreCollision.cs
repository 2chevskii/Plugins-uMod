...
namespace Oxide.Plugins
{
    ...
    internal class IgnoreCollision : RustPlugin
    {
        ...
        {
            Physics.IgnoreLayerCollision(26, 26, false);
            nowDisabled = false;
        }
        ...
        private void PrintConsoleWarning(WarningType warningType)
        {
            switch (warningType)
            {
                ...
                case WarningType.MoreThan:
                    if (logs)
                        PrintWarning(
                            $"Dropped item limit exceed ({ignoreStartAmount}) - collision disabled!"
                        );
                    break;
                case WarningType.LessThan:
                    if (logs)
                        PrintWarning(
                            $"Dropped items less than limit ({ignoreStartAmount}) - collision enabled!"
                        );
                    break;
                default:
                    break;
