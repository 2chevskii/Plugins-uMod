...
namespace Oxide.Plugins
{
    ...
    [Description(
        "Limits current maximum tickrate based on player count to improve overall performance"
    )]
    internal class TickrateLimiter : CovalencePlugin
    {
        ...
        private Dictionary<int, TickrateValues> GetDefaultCurve =>
            new Dictionary<int, TickrateValues>
            {
        ...
        {
            var closest = TickrateCurve
                .OrderByDescending(x => x.Key)
                .First(z => z.Key <= BasePlayer.activePlayerList.Count)
                .Value;
